...
- 파이썬이나 자바에서 볼 수 있는 해시 가능 객체라는 표현이 자바스크립트에는 없다.
  ...
  - 숫자도 키로 사용하려고 하면 문자열로 변환된다.
    - 배열은 객체이지만, 숫자 인덱스를 사용한다. 문자열도 가능하다.
...
- 배열을 순회할 때, 인덱스에 신경 쓰지 않는다면 `for ~ of`를 사용하는 것이 좋다.
- 만약 인덱스 타입이 중요하다면 `forEach`를 사용하면 된다.
- 루프 중간에 멈춰야 한다면 `for(;;)` 루프를 사용하는 것이 좋다.
- 타입이 불확실하다면 `for ~ in`이 `for ~ of` 또는 `for(;;)`에 비해 몇 배나 느리다.
